// Inner SVG markup (paths/shapes only) for the admin sidebar/topbar — hand
// picked from the Feather icon set, inlined so there's no extra dependency
// or network request for a handful of small stroke icons. Wrap with icon()
// in layout.ts to get a full <svg>.
export const ICONS = {
  dashboard:
...
// and styled consistently wherever an icon appears in the admin UI.
export function icon(paths: string, size = 18): string {
